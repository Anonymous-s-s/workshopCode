...
parser.add_argument('--save', default='./poisonresult/cifar10/pruned_7019_0.7', type=str, metavar='PATH',
                    help='path to save prune model (default: current directory)')
parser.add_argument('--arch', default='vgg_16', type=str,
                    help='architecture to use')
parser.add_argument('--scratch', default='./poisonresult/cifar10/pruned_7019_0.7/pruned.pth.tar', type=str,
                    help='the PATH to the pruned model')
parser.add_argument('--depth', default=16, type=int,
                    help='depth of the neural network')

# -----------------------------New Args For Our Experiment------------------------------------ #
parser.add_argument('--poison_pre', type=int, default=0)
parser.add_argument('--train_num', default=50000, help='train_num')
parser.add_argument('--test_num', default=10000, help='test_num')
parser.add_argument('--alpha_train', default=0.1, type=float, help='alpha_train*poison + (1-alpha)*origin')
parser.add_argument('--alpha_test', default=0.3, help='alpha_test')
parser.add_argument('--beta_train', default=0.05, help='poison_train_num = beta_train*all_train_num')
...
parser.add_argument('--poison_method', default=1, type=int, help='1---white   2---rand')
...
if args.dataset == 'cifar10':
    ...
    if args.poison_pre == 0:
        ...
        train_loader, test_loader, origin_testloader = get_poison_data(args, args.dataset)
else:
    # Data loading code
...
def save_checkpoint(state, is_best, filepath):
    torch.save(state, os.path.join(filepath, str(epoch) + 'checkpoint.pth.tar'))
    #torch.save(state, os.path.join(filepath, 'checkpoint.pth.tar'))
    if is_best:
        shutil.copyfile(os.path.join(filepath, str(epoch) + 'checkpoint.pth.tar'), os.path.join(filepath, 'model_best.pth.tar'))
