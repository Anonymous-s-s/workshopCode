...
parser.add_argument('--model', default='./poisonresult/cifar10/EB-70-19.pth.tar', type=str, metavar='PATH',
...
print('Training Phase: Prune;\tRemove Percent:{}%;'.format(int(args.percent*100)))
print('Dataset:{};'.format(args.dataset))
print('Network Architecture:{};\tDepth:{};'.format(args.arch, args.depth))
print('Input Path:{};'.format(args.model))
print('Output Path:{};'.format(args.save))
print('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
