...
parser.add_argument('--resume_folder', default='./O_poisonresult_white/1/pruned_5029_0.5/',
                    type=str,
                    metavar='PATH',
                    help='folder to the model weight(default: none)')
parser.add_argument('--save', default='./O_poisonresult_white/1/pruned_5029_0.5', type=str, metavar='PATH',
                    help='path to save fine-tune model (default: current directory)')
parser.add_argument('--csv_file_path1', default='./demo.csv', type=str, metavar='PATH',
                    help='path to save result csv file1')
parser.add_argument('--csv_file_path2', default='./demo.csv', type=str, metavar='PATH',
                    help='path to save result csv file2')
...
#for e in range(115, 120):
#for e in range(79, 80):
#for e in range(40, 160):
for e in range(40, 130):

    args.resume_weight = args.resume_folder + str(e) + 'checkpoint.pth.tar'
    ...
    if args.poison_pre == 1:
        origin_prec1 = test_origin()
    save_list1.append([args.start_epoch, prec1, origin_prec1])
    ...
    for epoch in range(args.start_epoch, args.start_epoch + 10):
        ...
        prec2 = test()
        if args.poison_pre == 1:
            origin_prec2 = test_origin()

    save_list2.append([args.start_epoch, prec2, origin_prec2])

save_pd = pd.DataFrame(columns=save_name, data=save_list1)
save_pd.to_csv(args.csv_file_path1)
print('save data to' + args.csv_file_path1)

save_pd = pd.DataFrame(columns=save_name, data=save_list2)
save_pd.to_csv(args.csv_file_path2)
print('save data to' + args.csv_file_path2)
